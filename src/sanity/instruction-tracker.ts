...
import type { TrackedInstruction, InstructionComplianceReport } from '../types';
...
export class InstructionTracker {
  ...
  extractInstructions(message: string): TrackedInstruction[] {
    ...
    const patterns = [
      // Must do patterns
      {
        regex: /(?:must|have to|need to|required to|always)\s+(.+?)(?:\.|$)/gi,
        priority: 'must' as const,
      },
      {
        regex: /(?:don't|do not|never|avoid)\s+(.+?)(?:\.|$)/gi,
        priority: 'must' as const,
      },
      {
        regex: /(?:make sure|ensure|verify)\s+(.+?)(?:\.|$)/gi,
        priority: 'must' as const,
      },

      // Should do patterns
      {
        regex: /(?:should|prefer|try to)\s+(.+?)(?:\.|$)/gi,
        priority: 'should' as const,
      },
      {
        regex: /(?:ideally|preferably)\s+(.+?)(?:\.|$)/gi,
        priority: 'should' as const,
      },

      // Nice to have patterns
      {
        regex: /(?:if possible|optionally|bonus if)\s+(.+?)(?:\.|$)/gi,
        priority: 'nice_to_have' as const,
      },
      {
        regex: /(?:would be nice|could also)\s+(.+?)(?:\.|$)/gi,
        priority: 'nice_to_have' as const,
      },
    ];
  ...
  extractFromPlan(
    ...
    for (const task of tasks) {
      const instruction = this.addInstruction(task, 'approved_plan', 'must');
      extracted.push(instruction);
    }
  ...
  verifyCompliance(output: string, changedFiles: string[]): void {
    ...
    // changedFiles parameter kept for API compatibility
    void changedFiles;
    ...
    for (const [id, instruction] of this.instructions) {
      ...
      const isNegation = /^(?:don't|do not|never|avoid|no)\s/i.test(
        instruction.text,
      );

      if (isNegation) {
        // For negations, we need to verify the thing isn't done
        const forbiddenContent = textLower.replace(
          /^(?:don't|do not|never|avoid|no)\s+/i,
          '',
        );
  ...
  generateReport(): InstructionComplianceReport {
    ...
    const notApplicable = all.filter(
    ...
    const overallScore =
      totalPossible > 0 ? Math.round((totalEarned / totalPossible) * 100) : 100;
