...
import { BaseValidator, ValidatorContext, extractImports, extractFunctionCalls } from './base-validator';
...
export class ApiExistenceValidator extends BaseValidator {
  ...
  protected async validate(context: ValidatorContext): Promise<ValidationIssue[]> {
    ...
    for (const change of context.changes) {
      ...
      for (const call of calls) {
        const fullCall = call.object ? `${call.object}.${call.call}` : call.call;
  ...
  private isLikelyHallucination(object: string, method: string): boolean {
    ...
    const invalidArrayMethods = ['contains', 'remove', 'first', 'last', 'isEmpty', 'clear'];
    const invalidStringMethods = ['contains', 'isEmpty', 'toArray', 'chars'];
    const invalidObjectMethods = ['hasKey', 'getKeys', 'getValues'];

    if (['array', 'arr', 'list', 'items'].some(n => object.toLowerCase().includes(n))) {
      if (invalidArrayMethods.includes(method)) return true;
    }

    if (['string', 'str', 'text', 'name'].some(n => object.toLowerCase().includes(n))) {
      if (invalidStringMethods.includes(method)) return true;
    }

    if (['object', 'obj', 'data'].some(n => object.toLowerCase().includes(n))) {
...
export class DependencyVerifierValidator extends BaseValidator {
  ...
  private isNodeBuiltin(name: string): boolean {
    const builtins = [
      ...
      'crypto', 'buffer', 'querystring', 'url', 'child_process', 'cluster',
      'dgram', 'dns', 'domain', 'net', 'readline', 'repl', 'tls', 'tty',
      'vm', 'zlib', 'assert', 'console', 'process', 'timers', 'module',
      'node:fs', 'node:path', 'node:crypto' // Node 16+ prefixed imports
    ];
...
export class StyleMatcherValidator extends BaseValidator {
  ...
  protected async validate(context: ValidatorContext): Promise<ValidationIssue[]> {
    ...
    for (const change of context.changes) {
      ...
      issues.push(...this.compareStyles(existingStyle, changeStyle, change.path));
    }

    return issues;
  }

  private detectProjectStyle(files: Map<string, string>): Record<string, string> {
  ...
  private compareStyles(
    ...
    if (project['quotes'] && change['quotes'] && project['quotes'] !== change['quotes']) {
      issues.push(this.createIssue(
        filePath,
    ...
    if (project['semicolons'] && change['semicolons'] && project['semicolons'] !== change['semicolons']) {
      issues.push(this.createIssue(
        filePath,
    ...
    if (project['indent'] && change['indent'] && project['indent'] !== change['indent']) {
      issues.push(this.createIssue(
        filePath,
...
export class ComplexityAnalyzerValidator extends BaseValidator {
  ...
  protected async validate(context: ValidatorContext): Promise<ValidationIssue[]> {
    const issues: ValidationIssue[] = [];
    const maxComplexity = (this.options.maxCyclomaticComplexity as number) ?? 10;
    const maxFunctionLength = (this.options.maxFunctionLength as number) ?? 50;

    for (const change of context.changes) {
      issues.push(...this.analyzeComplexity(change.content, change.path, maxComplexity, maxFunctionLength));
    }
  ...
  private extractFunctions(content: string): Array<{ name: string; body: string; startLine: number }> {
    const functions: Array<{ name: string; body: string; startLine: number }> = [];
    const regex = /(?:async\s+)?(?:function\s+(\w+)|(?:const|let|var)\s+(\w+)\s*=\s*(?:async\s+)?(?:\([^)]*\)|[^=])\s*=>|(\w+)\s*\([^)]*\)\s*{)/g;
  ...
  private hasAsyncOperation(code: string): boolean {
    return /await\s+|\.then\s*\(|fetch\s*\(|axios\.|\.subscribe\s*\(/.test(code);
  }
...
export class EdgeCaseCheckerValidator extends BaseValidator {
  ...
  private checkEdgeCases(content: string, filePath: string): ValidationIssue[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (/\w+\[\w+\]/.test(line) && !/\.length|\.at\(|if\s*\(/.test(lines.slice(Math.max(0, i - 2), i + 1).join('\n'))) {
      ...
      if (/\/\s*\w+[^/]/.test(line) && !/if\s*\(.*===?\s*0|!==?\s*0/.test(lines.slice(Math.max(0, i - 3), i + 1).join('\n'))) {
        if (!/\/\s*\d+/.test(line)) { // Skip constant division
          issues.push(this.createIssue(
            filePath,
            `Division without zero check`,
            'info',
            { line: i + 1, code: 'EDGE_DIVISION_BY_ZERO' }
          ));
        }
      }

      // Object property access on potentially null value
      if (/\w+\.\w+/.test(line) && /\|\|\s*null|\?\s*:/.test(lines.slice(Math.max(0, i - 2), i).join('\n'))) {
        if (!/\?\.\w+/.test(line)) { // Not using optional chaining
          issues.push(this.createIssue(
            filePath,
      ...
      if (/JSON\.parse\s*\(/.test(line)) {
        if (!/try\s*{/.test(lines.slice(Math.max(0, i - 3), i + 1).join('\n'))) {
          issues.push(this.createIssue(
            filePath,
...
export class RefactorCompletenessValidator extends BaseValidator {
  ...
  private detectRenames(context: ValidatorContext): Array<{ file: string; oldName: string; newName: string }> {
