...
export class HooksAgent extends BaseAgent {
    ...
    try {
      ...
      const response = await this.callApi({
        ...
        messages: this.buildMessages(
          this.buildHooksPrompt(task, context),
          context,
        ),
    ...
    while ((match = effectRegex.exec(change.content)) !== null) {
      ...
      if (usesState) {
        ...
        issues.push({
          ...
          line: lineNum,
          message:
            'useEffect has empty dependency array but may use external values',
          severity: 'warning',
  ...
  private checkEventCleanup(change: FileChange): ValidationIssue[] {
    ...
    const addListenerMatches =
      content.match(/addEventListener\s*\(['"]\w+/g) ?? [];
    const removeListenerMatches =
      content.match(/removeEventListener\s*\(['"]\w+/g) ?? [];
    ...
    const clearIntervalCount = (content.match(/clearInterval\s*\(/g) ?? [])
      .length;
