...
export interface ContextSource {
  type: 'file' | 'memory' | 'conversation' | 'plan' | 'assumption' | 'instruction';
  content: string;
  priority: number;
  tokens: number;
...
export class ContextAssembler {
  ...
  async assemble(params: {
    ...
    const availableTokens = this.budget.maxTokens - this.budget.reserveForResponse;
    ...
    const budgets = {
      conversation: Math.floor(availableTokens * this.budget.priorityAllocation.conversation / 100),
      memory: Math.floor(availableTokens * this.budget.priorityAllocation.memory / 100),
      files: Math.floor(availableTokens * this.budget.priorityAllocation.files / 100),
      plan: Math.floor(availableTokens * this.budget.priorityAllocation.plan / 100)
    };

    // Assemble each category within budget
    const conversation = this.trimConversation(params.conversationHistory, budgets.conversation);
    const memory = this.trimMemory(params.activeMemory, budgets.memory);
    const files = this.trimFiles(params.relevantFiles, budgets.files, params.focusFiles);
    const plan = params.currentPlan ? this.trimPlan(params.currentPlan, budgets.plan) : undefined;
  ...
  calculateUsage(bundle: ContextBundle): {
    ...
    breakdown['memory'] = bundle.activeMemory.reduce(
      (sum, entry) => sum + estimateTokens(entry.summary + entry.detail), 0
    );

    breakdown['files'] = bundle.relevantFiles.reduce(
      (sum, file) => sum + estimateTokens(file.content), 0
    );
    ...
    const percentUsed = (total / (this.budget.maxTokens - this.budget.reserveForResponse)) * 100;
